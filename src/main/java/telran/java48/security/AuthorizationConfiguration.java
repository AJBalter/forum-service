        ...
        http.sessionManagement(Session -> Session.sessionCreationPolicy(SessionCreationPolicy.STATELESS));
       	http.authorizeRequests(authorize -> authorize
				...
				.mvcMatchers(HttpMethod.PUT, "/account/user/{login}")
					.access("#login == authentication.name and !hasRole('EXPIRED')")
				.mvcMatchers(HttpMethod.DELETE, "/account/user/{login}")
					.access("(#login == authentication.name and !hasRole('EXPIRED')) or hasRole('ADMINISTRATOR')")
				.mvcMatchers(HttpMethod.POST, "/forum/post/{author}")
				    .access("#author == authentication.name and !hasRole('EXPIRED')")
				.mvcMatchers(HttpMethod.PUT, "/forum/post/{id}/comment/{author}")
				    .access("#author == authentication.name and !hasRole('EXPIRED')")	
				.mvcMatchers(HttpMethod.PUT, "/forum/post/{id}")
				    .access("@customSecurity.checkPostAuthor(#id, authentication.name) and !hasRole('EXPIRED')")
				.mvcMatchers(HttpMethod.DELETE, "/forum/post/{id}")
				    .access("(@customSecurity.checkPostAuthor(#id, authentication.name) or hasRole('MODERATOR')) and !hasRole('EXPIRED')")
				.anyRequest()
					.authenticated()
		);
		return http.build();
